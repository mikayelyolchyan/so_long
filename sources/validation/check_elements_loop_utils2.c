...
/*   Updated: 2025/04/28 17:41:54 by miyolchy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (start->x == -1)
		return (ft_printf("Error: No player\n"), 0);
	if (total->coins < 1)
		return (ft_printf("Error: At least one coin required\n"), 0);
	if (total->power_ups != 0 && total->power_ups != 4)
		return (ft_printf("Error: Powerups must be 0 or exactly 4\n"), 0);
	if (total->exits != 1)
		return (ft_printf("Error: Exactly one exit required\n"), 0);
	if (total->portals != 0 && (total->portals != 2 || \
		portals[0] != 1 || portals[1] != 1))
		return (ft_printf("Error: Portals must be 0 or exactly \
			two at x=1 and x=width-2\n"), 0);
	if (total->ghost_count != 0 && total->ghost_count != 4)
		return (ft_printf("Error: Ghosts must be 0 or exactly 4\n"), 0);
