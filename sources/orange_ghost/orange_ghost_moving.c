...
void orange_ghost_moving(t_game *game)
{
        ...
        (game->o_ghost->direction == RIGHT && map[game->o_ghost->y / 32][(game->o_ghost->x + (game->o_ghost->is_eaten ? 8 : 4)) / 32 + 1] == '1'))
    {
        if (game->o_ghost->is_eaten == 0 &&
            (can_move_vertical(game->o_ghost, map, game->o_ghost->pending_direction) || 
             can_move_horizontal(game->o_ghost, map, game->o_ghost->pending_direction)))
        {
            game->o_ghost->direction = game->o_ghost->pending_direction;
        }
        else if (game->o_ghost->is_eaten == 1 && 
                 (can_move_fast_vertical(game->o_ghost, map, game->o_ghost->pending_direction) || 
                  can_move_fast_horizontal(game->o_ghost, map, game->o_ghost->pending_direction)))
        {
            game->o_ghost->direction = game->o_ghost->pending_direction;
        }
    ...
    if (game->o_ghost->is_eaten == 0)
    {
        ...
        {
            game->o_ghost->x -= 4;
            if (map[game->o_ghost->y / 32][(game->o_ghost->x - 4) / 32] == 'T')
            {
                mlx_put_image_to_window(game->mlx, game->win, game->map->black_wall_img, game->o_ghost->x, game->o_ghost->y);
                handle_ghost_portal(game, game->o_ghost, 1);
            }
        }
        else if (game->o_ghost->direction == RIGHT)
        {
            game->o_ghost->x += 4;
            if (map[game->o_ghost->y / 32][(game->o_ghost->x + 32) / 32] == 'T')
            {
                mlx_put_image_to_window(game->mlx, game->win, game->map->black_wall_img, game->o_ghost->x, game->o_ghost->y);
                handle_ghost_portal(game, game->o_ghost, 0);
            }
        }
    }
    else if (game->o_ghost->is_eaten == 1)
    {
        ...
        {
            game->o_ghost->x -= 16;
            if (map[game->o_ghost->y / 32][(game->o_ghost->x - 16) / 32] == 'T')
            {
                mlx_put_image_to_window(game->mlx, game->win, game->map->black_wall_img, game->o_ghost->x, game->o_ghost->y);
              	handle_ghost_portal(game, game->o_ghost, 1);
            }
